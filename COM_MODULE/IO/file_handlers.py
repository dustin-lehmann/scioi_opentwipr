from typing import List, Dict, Tuple, Set, Optional, Union, Sequence, Callable, Iterable, Iterator, Any, AnyStr, TextIO, BinaryIO
import io
import os.path
import os
import csv
            ...
            with open(filepath) as csv_file:
                ...
                for row in reader:
                    if reader.line_num == 0:
                        fieldnames = list(dict(row).keys())
                        if fieldnames not in valid_field_name_combinations:
                            raise ValueError
                        input_data = []
                        continue
