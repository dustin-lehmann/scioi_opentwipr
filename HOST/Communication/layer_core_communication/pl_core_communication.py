...
def pl_tx_handling(pl_ml_tx_queue: Queue(), tx_queue: Queue()):
    ...
    - check if there are any data that is supposed to be sent, if so put translated msg in queue for next layer
...
def pl_rx_handling(rx_queue: Queue(), pl_ml_rx_queue: Queue()):
    ...
    - check if there are any data that is supposed to be sent, if so put raw-msg in queue for next layer
